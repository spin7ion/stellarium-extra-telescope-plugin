...
//! queues a set time command
void TelescopeClientDirectNexStar::telescopeSetTime()
{
	if (!isConnected())
		return;
	...
	nexstar->sendTime(ct.hour(), ct.minute(), ct.second(),
		cd.month(), cd.day(), cd.year(),
		3, (unsigned int)tz.hasDaylightTime());
}
