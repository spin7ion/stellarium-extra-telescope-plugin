...
NexStarCommandSetTime::NexStarCommandSetTime(Server &server, unsigned int Q_int, unsigned int R_int, unsigned int S_int, unsigned int T_int, unsigned int U_int, unsigned int V_int, unsigned int W_int, unsigned int X_int) : NexStarCommand(server)
{
	Q=Q_int;
	R=R_int;
	S=S_int;
...
bool NexStarCommandSetTime::writeCommandToBuffer(char *&p,char *end)
{
	#ifdef DEBUG5
	char *b = p;
	#endif
...
/*
	...
	*p = 0;
	...
	#ifdef DEBUG5
	*log_file << Now() << "NexStarCommandSetTime::writeCommandToBuffer:"
...
int NexStarCommandSetTime::readAnswerFromBuffer(const char *&buff, const char *end) const
{
	if (buff >= end)
		return 0;
	...
	{
		#ifdef DEBUG4
		*log_file << Now() << "NexStarCommandSetTime::readAnswerFromBuffer: set time ok"
	...
	{
		#ifdef DEBUG4
		*log_file << Now() << "NexStarCommandSetTime::readAnswerFromBuffer: set time failed." << StelUtils::getEndLineChar();
		#endif
	}
	buff++;
	return 1;
...
void NexStarCommandSetTime::print(QTextStream &o) const
{
	o << "NexStarCommandSetTime("
	  << Q << ":" << R << ":" << S << " "
	  << T << "/" << U << "/" << V << " "
	  << W << ";" << X
	<<')';
